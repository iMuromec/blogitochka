...
>(function AlertCancel({ className, ...props }, ref) {
  return (
    <AlertDialogPrimitives.Cancel
      ref={ref}
      className={cn(
        "relative inline-flex h-9 items-center rounded-md border border-slate-200 bg-white px-4 py-2 text-sm font-medium hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2",
        className
      )}
      {...props}
    />
...
interface AlertActionProps
  extends AlertDialogPrimitives.AlertDialogActionProps {
  onClick: () => void;
}

Alert.Action = React.forwardRef<HTMLButtonElement, AlertActionProps>(
  function AlertAction({ className, ...props }, ref) {
    return (
      <AlertDialogPrimitives.Action
        ref={ref}
        className={cn(
          "relative bg-red-600 focus:ring-red-600 inline-flex h-9 items-center rounded-md border border-transparent px-4 py-2 text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2",
          className
        )}
        {...props}
      />
